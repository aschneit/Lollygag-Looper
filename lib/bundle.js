...
const AppView = __webpack_require__(2);
...
const { Instrument } = __webpack_require__(3);
  ...
  loopDrums () {
    ...
    this.canvas.addEventListener('click', (e) => {
      ...
      if (this.isIntersection(pos, drums)) {
        ...
        this.drumsMute.on('change', (event) =>{
          if (this.drumsMute.prop("checked")) {
            drumLoop.mute = true;
          } else {
            drumLoop.mute = false;
          }
        });
        if (!this.drumsMute.prop("checked")) drumLoop.start(0);
  ...
  loopKeyboard() {
    ...
    keys.forEach((key) => {
      if (pos.x > key.xStart && pos.x < key.xEnd && pos.y > key.yStart && pos.y< key.yEnd) {
        ...
        const displace = Tone.Transport.seconds % (60 / Tone.Transport.bpm.value * 4);
        if (this.looping === true && !this.keysMute.prop("checked")) keyboardLoop.start(displace);
        this.stopLoop(keyboardLoop);
        const keysMute = $('.keys-mute');
        this.keysMute.on('change', (event) =>{
          if (this.keysMute.prop("checked")) {
...
/* harmony export (immutable) */ __webpack_exports__["Instrument"] = Instrument;
